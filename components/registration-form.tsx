...
export function RegistrationForm({ selectedCar, onSelectCar }: RegistrationFormProps) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      // Send to local secure Resend API route
      ...
      if (response.ok && result.success) {
        // Store in localStorage with generated ID for dashboard compatibility
        ...
        // Smooth redirect to the premium receipt confirmation page
        setTimeout(() => {
      ...
      console.error("Error sending to registration API:", error)
      alert("حدث خطأ أثناء إرسال طلبك. يرجى التحقق من الاتصال بالإنترنت والمحاولة مجدداً.")
    } finally {
      setIsLoading(false)
    }
  }
